...
#
# THIS FILE IS A VERBATIM COPY, and that is the deliberate shape rather than an accident. Its origin is
# `hooks/scripts/agents-md.test.sh` in `tadeumendonca-skills`, which carries the same artifact under the
# same rule and needs the same four arms. It derives its root from git rather than from its own position
# in the tree, so it runs unchanged from any depth in either repository.
#
# THE COST, NAMED RATHER THAN DISCOVERED LATER: nothing makes the two copies move together. A repository
# cannot run a script that lives in another repository's checkout, and this platform's rule is that
