...
const { generateOgImage } = vi.hoisted(() => ({ generateOgImage: vi.fn(async () => new Uint8Array([0x89, 0x50])) }));

vi.mock('../../../shared/db/client', () => ({ ddb: { send } }));
vi.mock('../../../shared/s3/client', () => ({ objectExists, putImage }));
vi.mock('../generator', () => ({ generateOgImage }));
...
describe('GET /og/{type}/{slug}.png', () => {
  ...
  it('404s for an unsupported type', async () => {
    const res = await app.request('/og/posts/whatever.png');
