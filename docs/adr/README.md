...
| [0040](./0040-build-time-mermaid-diagrams.md) | Diagrams are Mermaid in the markdown body, compiled to inline SVG at build time (committed artifact, keyed by normalised source; Playwright not mermaid-cli, forced by ADR-0021's `--ignore-scripts`; failure is a red build, never a render-time fallback) | accepted · amends [0010](./0010-routing-landing-cv-split-redirects.md) (the orientation claim) and [0008](./0008-brutalist-mono-identity.md) (a generated surface) · **slice one of two** — the dev-loop diagram and the layout question are slice two |
