...
| [0043](./0043-harness-inventory-derived-from-plugin-repo.md) | The dev-loop inventory on `/architecture` is **derived from the sibling plugin repo** — a committed manifest generated from `tadeumendonca-skills` plus a drift check (missing · orphaned · changed) run in `app` against a second, **tokenless** checkout of that public repo; the build itself never reads it. Also decides what the diagram may assert: `PreToolUse` hooks **deny** (hooks are classed by **event** — the `SessionStart` pair only **documents**), personas only **advise** — a claim about their judgement, not a denial that `quality-assurance` holds a hook-enforced merge seat — and command families **document**; a closed set, unknown values throw. The CI gates deny too and are deliberately **not** manifest rows: they live in this repo, not in the plugin | accepted · same shape as [0040](./0040-build-time-mermaid-diagrams.md) + the decision index (`adr-source.mjs`) · constrained by [0002](./0002-fully-static-spa-no-backend.md) / [0004](./0004-build-time-render-not-ssr-or-edge.md) · applies [0015](./0015-oidc-immutable-subject-least-privilege.md) to a CI path filter (stays **out** of `deploy`'s gate) · **the first cross-repo build dependency in a repo whose architecture is "content ships in the repo"** — accepted costs: the check is **asymmetric** (a plugin PR cannot see this page, so the red lands late on an unrelated author) and it is **not self-triggering**, since no path in this repo can match the event that falsifies it. Motivating evidence: #318's own spec said 19 personas and 3 hooks; the tree says **6** and **4** · **amended 2026-08-04** (the plugin's version is resolved at **deploy** time, with a committed `plugin-release.json` as the floor — #329) · **amended 2026-08-04 again** (`marketing-lead` merged into `product-lead` and `new-issue` joined the root commands, so the published inventory is **5 personas** and **2** un-namespaced commands; the dated 19→6 table is left standing as evidence about that day. Adds the guard this exposed: the components test asserted *manifest ⊆ drawing* only, so a **retired** persona left in the fence passed every check — the drift job compares the manifest to the plugin, never the page to the manifest) |
