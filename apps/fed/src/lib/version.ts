...
// the amendment for why and for what un-pinning requires.~~
//
...
//
// STRUCK 2026-08-21 — THE PIN IS GONE. The owner reversed the 2026-08-14 amendment ("deveria ser
// atualizado automaticamente a cada deploy"), and `deploy.yml`'s `plugin-checkout` step now carries no
// `ref:`, so level 1 reads tadeumendonca-skills' default branch again — which is its freshest RELEASED
// tag, since every merge there bumps VERSION and cuts the tag in one commit. The two paragraphs above
// are struck rather than deleted because they were read and acted on. An ADR-0043 amendment recording
// the reversal is OWED and is `tech-lead`'s to write.
//
// SO THE TWO LEVELS NO LONGER AGREE BY CONSTRUCTION, and that is the normal state, not a defect: level 1
// is whatever the deploy resolved, level 2 is whatever the last `gen-harness` run committed, and level 2
// is BEHIND whenever the plugin has released since. `check-harness-drift` only reddens on the impossible
// direction — a committed floor AHEAD of the plugin, which names a release that does not exist and would
// 404 the card. Behind is silent by design.
//
// This module's own resolution mechanism (`resolvePluginVersion` below) was unchanged by the pin and is
// unchanged by its removal — it still just prefers level 1 over level 2.
