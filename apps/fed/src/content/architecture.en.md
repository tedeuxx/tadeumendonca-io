...
And the version stamped on all of it is content rather than release plumbing: what moves each digit is a decision on record, which is the sort of thing this page links rather than restates.

*(→ [ADR-0044](https://github.com/tedeuxx/tadeumendonca-io/blob/main/docs/adr/0044-version-parts-deliberate-major-minor.md) version digits carry content meaning)*
...
**Naming the orchestrator added a claim with no diagram behind it, and this release added six more, so what is counted here is claims rather than pictures — eleven now: eight you can check, and three you cannot.** Five of the six new ones resolve: the feature list to routes you can open and the decisions beside them, the publication scaffold to a script with its own tests, the roster to the plugin's own record, the documentation index to a live tree, and what the version digits mean to the record that decides it. The second uncheckable one is the orchestrator and everything said about it, and it fails to resolve in two layers. The actor itself is in no diagram here and in no inventory — it is not a plugin component, so the drift check that keeps the harness figure honest is structurally unable to see it, and nothing here goes red when that description stops being true. And the reading beside it — stated as a floor precisely because of this — came from session transcripts that sit on my machine and are not published, so what you have is testimony with a method attached rather than something you can re-run.
