...
### The part that does not arrive in the box

And there is one component in none of that — not in the drawing, not in the manifest, not in the plugin at all. The **orchestrator** is the main session: the context that reads an Issue, decides which persona to dispatch, weighs what comes back, and runs the merge. Install the plugin and you get hooks, personas and commands; the thing that assembles them is the half you write yourself, and anyone deciding whether to adopt this should know which half is a download. It is also the party the capability boundaries above are drawn *against* — `permission-guard` refuses the merge command from every agent type except `quality-assurance`, and the gloss on the persona edge, *advises, if dispatched*, names dispatch as the failure mode without naming who dispatches. That is the orchestrator, and a lens it forgets is a lens nobody ran.

Why the roster is **five** rather than nineteen is a decision with a record, so the rule this page runs on applies: link it rather than restate it. The cut happened in one amendment, and the criterion behind it was widened in a later one — the record now names **four** reasons a persona may exist at all, and one of them is that the orchestrator's context is a finite resource the design spends deliberately. [ADR-0002's amendments in the plugin repo](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/docs/adr/0002-agentic-dev-loop-architecture.md) carry both moves and what each cut cost. The plural matters: *"five because of the context window"* is a simplification the file itself refuses.

That resource has been read once, and the honest form of the reading is a **floor**, not a figure. Measured on this repo's own session of 7–8 August 2026, by parsing the session transcripts: **over an order of magnitude** more context stayed inside the subagents than came back as verdicts. The saving is real and it is bounded — the session **compacted twice** anyway, and the returned verdicts were a **large minority** of everything the orchestrator took in from a tool.

**Point estimates are deliberately not printed here, and the reason is this page's own standard rather than modesty.** Re-read twenty-six minutes later every count had moved, because the population is still growing while you measure it; and there are four different denominators in play — dispatches, transcripts, agents that returned, returns — so any single ratio has to pick one of them silently. More decisive than either: **the input is a private session transcript on one machine.** It is in neither repository, no gate can reach it, and nothing on this page re-derives it. That makes it a reading and not an artifact you can check — and a page arguing that claims resolve to files owes you that sentence when one does not.

What survives the arithmetic is the shape, which is a design claim rather than a measurement:

> A task costs the orchestrator its **verdict**, not its **execution**. That is a bound and not an escape — the verdicts accumulate anyway, and this session compacted twice regardless.

Which is what turns a property into a knob, and the knob needs no number either. The saving is bounded by how long the verdicts are, and that is the one term the harness controls — by writing shorter persona briefs.
...
**Naming the orchestrator moved that tally, so here it is again: three you can check, and now two you cannot.** The second is the orchestrator and everything said about it, and it is unverifiable in two layers. The actor itself is in no diagram and in no inventory — it is not a plugin component, so the drift check that keeps the harness figure honest is structurally unable to see it, and nothing here goes red when that description stops being true. And the reading beside it — stated as a floor precisely because of this — came from session transcripts that sit on my machine and are not published, so what you have is testimony with a method attached rather than something you can re-run. Written down rather than left implied, because it changes a count this page prints about itself.
