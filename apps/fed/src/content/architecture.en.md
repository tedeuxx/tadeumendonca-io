...
flowchart LR
  ...
  accDescr: A grid: four lanes, one per kind of component the plugin exports, crossed with three columns, one per class of force. The lanes are the 7 hooks registered in hooks.json, the 6 personas in the agents directory, the 13 skills in the skills directory, and the 3 commands in the commands directory. Of the twelve cells, five hold something and seven are empty — and the empty ones are the drawing's claim. The deny column holds exactly one cell: permission-guard and wip-guard, the 2 hooks on the PreToolUse event with the matcher Bash, which REFUSE a tool call before it runs; persona, skill and command have no cell there at all. The middle column holds one too: the personas agents-lead, developer, product-lead, quality-assurance, tech-lead and writer only ADVISE, which is a claim about their JUDGEMENT and not about their seat. quality-assurance is the sharpest case in both directions: permission-guard rule 7b refuses the merge command from every agent type except that one, so WHO merges is mechanically forced — and nothing anywhere checks whether the review was performed, or performed well. agents-lead is weaker still, and must not be read as the same thing: it runs before anything is built rather than after, it gates nothing, and nothing forces it to be dispatched at all — a lens that is not dispatched fails silently. product-lead is the mirror case: it BLOCKS a merge when it finds a published claim that is untrue, but by convention rather than by hook, so nothing refuses the merge command on its behalf; and writer, the persona that drafts published prose, is contained by the same mechanism product-lead is — a permission-guard rule denies it posting directly, since it reads private material to draft. The third column, the one whose components only DOCUMENT, holds three cells: the 13 skills, which is what the model reaches for on its own, the 3 commands, which is what a person types, and the other 5 hooks — session-wip and session-plugin-version on SessionStart, dispatch-metrics-start and dispatch-metrics-stop on SubagentStart and SubagentStop, and zombie-loop-detect on Stop. Four of those five sit in that column because their three events hand a hook no tool call to refuse: the column states what a hook on one of those events cannot STOP rather than promising that it merely watches — a hook on any of them runs and can act, it simply has no tool call in front of it to deny. The fifth is the sharper case, and it is the reason the events counted here are four and not three: Stop CAN refuse — Claude Code lets a hook on that event block the turn from ending — and zombie-loop-detect declines to. It fires at the end of every turn of mine and reads one thing: does the current branch have an open pull request whose last gate verdict, matched against that pull request's current head, asks for changes or waits on my go. If it does, the notice opens the next turn; every exit path is a success and nothing is ever blocked. So that hook sits in the DOCUMENT column by its own CHOICE rather than by the event's limits, and what it buys is detection one turn late, not prevention: a dispatch that was narrated and never made leaves the loop stalled with no artifact saying so, and this is the artifact that says so. All five only report, and that is a fact about each script, not a property of the event. Read end to end, the grid is mostly empty, and that emptiness is what it asserts: of everything the plugin exports, exactly one kind — a hook on PreToolUse — can refuse; everything else advises or documents.
  ...
  subgraph COLO["DOCUMENTS"]
    direction TB
    HKR["5 hooks · none of them refuses<br/>2 hooks · SessionStart<br/>session-wip<br/>session-plugin-version<br/>2 hooks · SubagentStart and SubagentStop<br/>dispatch-metrics-start<br/>dispatch-metrics-stop<br/>1 hook · Stop<br/>zombie-loop-detect<br/>could block the turn — chooses not to"]
    PSO["— no persona"]
...
**Of the plugin's own components, exactly one kind can stop you**, and that is the honest version of the adoption pitch: the two `PreToolUse` hooks return a denial *before* the tool runs, and the command does not happen. The other five only report — four of them because their events hand them no tool call to refuse, and the fifth because it declines to: `zombie-loop-detect` runs on `Stop`, an event that *can* block the turn from ending, and every one of its exit paths is a success. It notices, one turn late, that a branch is sitting on a gate verdict nobody acted on. That is a hook choosing not to be a mechanism, which is a distinction this page owes you rather than one it can round off. And the personas **advise** — their judgement is checked by nothing, and this repo's own guide says in as many words that a lens nobody dispatches *fails silently*. That is the guarantee the loop gives — and it is worth exactly what the inventory in the drawing above is worth.
