...
Why the roster is **five** rather than nineteen is a decision with a record, so the rule this page runs on applies: link it rather than restate it. The cut was made on four stated reasons, and one of them is that the orchestrator's context is a finite resource the design spends deliberately — [ADR-0002's amendments in the plugin repo](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/docs/adr/0002-agentic-dev-loop-architecture.md) carry the criterion, the reasons and what each cut cost. The plural matters: *"five because of the context window"* is a simplification the file itself refuses.

That resource has now been measured once, which is more than this page could say before. On this workspace's session of 7–8 August 2026, read by parsing the session transcripts: **125 dispatches** — `quality-assurance` 45, `product-lead` 40, `tech-lead` 29, `harness-reviewer` 7, `developer` 2 — kept roughly **2.78 million tokens inside the subagents** across 4,886 tool uses, and returned about **159,000 tokens of verdict text**: 82 returns, some 99,000 words. Aggregate ratio **17×**; per dispatch the median is **19×**. Returns were around **44%** of everything the orchestrator took in from a tool.

> A task costs the orchestrator its **verdict**, not its **execution** — measured here at about a twentieth. That is a bound, not an escape: 125 dispatches returned ~99,000 words, and the session still compacted twice.

Which is what turns a property into a knob. The saving is bounded by the verbosity of the returns, and that is the one term the harness controls — by writing shorter persona briefs.

Every number above is **one reading**: one session, one repository, one day, one shape of the roster. It is not a property of the design, nothing on this page re-derives it, and the transcripts it came from are local and unpublished. Treat it the way the bill further up is treated — a measurement with a date on it, not a standing fact.
...
**Naming the orchestrator moved that tally, so here it is again: three you can check, and now two you cannot.** The second is the orchestrator and everything said about it, and it is unverifiable in two layers. The actor itself is in no diagram and in no inventory — it is not a plugin component, so the drift check that keeps the harness figure honest is structurally unable to see it, and nothing here goes red when that description stops being true. And the dispatch figures beside it were read once, from session transcripts that sit on my machine and are not published, so what you have is testimony with a method attached rather than something you can re-run. Written down rather than left implied, because it changes a count this page prints about itself.
