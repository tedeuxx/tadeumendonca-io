...
**Of the plugin's own components, exactly one kind can stop you**, and that is the honest version of the adoption pitch: three of the thirteen [registrations in `hooks.json`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/hooks.json) return a denial *before* the act, and the act does not happen. They do it in **two different shapes**, which is the part worth having: [`permission-guard`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/permission-guard.sh) on `Bash` and [`mcp-guard`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/mcp-guard.sh) on the MCP tools sit on `PreToolUse` and refuse a **tool call**; [`preflight`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/preflight.sh) sits on `UserPromptSubmit` and refuses the **prompt**, before the turn starts at all. ~~the three `PreToolUse` hooks — `permission-guard` and `wip-guard` on `Bash`, and `orchestrator-write-guard` on the file-writing tools~~ — **struck: of those four nouns, one survives.** Two of those guards were deleted in the plugin and `mcp-guard` had never appeared on this page at all. And `preflight` is drawn in **two** columns of the grid above, because it is registered twice with a different class on each event — which is the one thing here nobody authoring by hand would have drawn.

The other ten registrations only report, and not for the same reason — a distinction this page owes you rather than one it can round off. **Four** of them sit on events that refuse nothing, `SessionStart` and `SubagentStart`, so those four could not block if they wanted to; note that the reason is the **event**, not the absence of a tool call, because `UserPromptSubmit` has no tool call in front of it either and denies anyway. **The other six could.** `Stop` *can* block the turn from ending and `SubagentStop` *can* block a subagent from stopping, and every one of those scripts declines: every one of their exit paths is a success. [`zombie-loop-detect`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/zombie-loop-detect.sh) notices, one turn late and once per session for that head, that a branch is sitting on a gate verdict nobody acted on; [`dispatch-metrics-stop`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/dispatch-metrics-stop.sh) files the dispatch's numbers and gets out of the way; [`orchestrator-tool-census`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/orchestrator-tool-census.sh) names what the orchestrator did with its own hands, write and post calls apart from reads, and decides nothing about it. Six hooks choosing not to be mechanisms says more about this harness than one did. And the personas **advise** — their judgement is checked by nothing, and this repo's own guide says in as many words that a lens nobody dispatches [*fails silently*](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/commands/autonomy.md). That is the guarantee the loop gives — and it is worth exactly what the inventory in the drawing above is worth.
...
|---|---|---|---|
| `product-lead` | the reader, value, order, slice size — and positioning, voice, and the truth of anything published | `tech-lead`; and it **blocks a merge** when it finds a published claim that is untrue — by convention rather than by hook | `agents-configuration` · `engineering-standards` · `definition-of-ready` · `shell` |
