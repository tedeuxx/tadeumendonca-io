...
flowchart TB
  ...
  subgraph AFK["AFK · from ready to merge, nothing on the path is human"]
    ...
    QA["TIER 3 · GATE<br/>— fresh context, no authorship bias<br/>quality-assurance · the only one that may merge<br/>product · content — the Definition of Done,<br/>and whether this breaks production<br/>loop — the agents-lead verdict marker instead"]
...
flowchart LR
  ...
  accDescr: A grid: four lanes, one per kind of component the plugin exports, crossed with three columns, one per class of force. The lanes are the 9 hooks registered in hooks.json, the 7 personas in the agents directory, the 14 skills in the skills directory, and the 3 commands in the commands directory. Of the twelve cells, five hold something and seven are empty — and the empty ones are the drawing's claim. The deny column holds exactly one cell: the 3 hooks on the PreToolUse event, which REFUSE a tool call before it runs — permission-guard and wip-guard on the matcher Bash, and orchestrator-write-guard on the file-writing tools, Edit, Write, MultiEdit and NotebookEdit; persona, skill and command have no cell there at all. The middle column holds one too: the personas agents-lead, content-reviewer, content-writer, developer, product-lead, quality-assurance and tech-lead only ADVISE, which is a claim about their JUDGEMENT and not about their seat. quality-assurance is the sharpest case in both directions: permission-guard rule 7b refuses the merge command from every agent type except that one, so WHO merges is mechanically forced — and nothing anywhere checks whether the review was performed, or performed well. agents-lead is weaker still, and must not be read as the same thing: it runs before anything is built rather than after, it gates nothing, and nothing forces it to be dispatched at all — a lens that is not dispatched fails silently. product-lead is the mirror case: it BLOCKS a merge when it finds a published claim that is untrue, but by convention rather than by hook, so nothing refuses the merge command on its behalf; and the content pair — content-writer, which drafts published prose, and content-reviewer, which reads that draft against the same ruler — is contained by the same mechanism product-lead is: a permission-guard rule denies each of them posting directly, since both read private material to do their job. The third column, the one whose components only DOCUMENT, holds three cells: the 14 skills, which is what the model reaches for on its own, the 3 commands, which is what a person types, and the other 6 hooks — session-wip and session-plugin-version on SessionStart, dispatch-metrics-start and dispatch-metrics-stop on SubagentStart and SubagentStop, and zombie-loop-detect and orchestrator-tool-census on Stop. Those six do not share one reason for being there, and saying which reason applies to which is the sharper claim. Three of them cannot refuse anything: SessionStart and SubagentStart hand a hook no tool call to deny, so for session-wip, session-plugin-version and dispatch-metrics-start the column states a limit of the EVENT — a hook on either event runs and can act, it simply has nothing in front of it to stop. The other three could and do not. Stop and SubagentStop both CAN refuse — Claude Code lets a hook on the first block the turn from ending and a hook on the second block the subagent from stopping — and no script here takes it. dispatch-metrics-stop writes the dispatch's metrics to its Issue and exits successfully on every path, including every path where it gives up early. zombie-loop-detect fires at the end of every turn of mine and reads one thing: does the current branch have an open pull request whose last gate verdict, matched against that pull request's current head, asks for changes or waits on my go. If it does, the notice opens the next turn once per session for that head: it does not repeat while nothing moves, it re-arms when new commits land, and a second session picking the same branch up says it once more; every exit path is a success and nothing is ever blocked. orchestrator-tool-census is the newest of them and the plainest instrument: at the end of a turn it tells me what the orchestrator did with its own hands this session, as a named list with the write and post calls separated from the reads, and it gates nothing because a Stop hook fires after the work already happened. So those three sit in the DOCUMENT column by their own CHOICE rather than by the event's limits, and what zombie-loop-detect buys is detection one turn late, not prevention: a dispatch that was narrated and never made leaves the loop stalled with no artifact saying so, and this is the artifact that says so. All six only report — three because the event allows nothing else, three because the script decided so, and the column carries both without pretending they are one thing. Read end to end, the grid is mostly empty, and that emptiness is what it asserts: of everything the plugin exports, exactly one kind — a hook on PreToolUse — can refuse; everything else advises or documents.
  ...
  subgraph COLO["DOCUMENTS"]
    direction TB
    HKR["6 hooks · none of them refuses<br/>2 hooks · SessionStart<br/>session-wip<br/>session-plugin-version<br/>2 hooks · SubagentStart and SubagentStop<br/>dispatch-metrics-start<br/>dispatch-metrics-stop<br/>2 hooks · Stop<br/>zombie-loop-detect<br/>orchestrator-tool-census<br/>Stop and SubagentStop could block — none of the three does"]
    PSO["— no persona"]
...
**Of the plugin's own components, exactly one kind can stop you**, and that is the honest version of the adoption pitch: the three `PreToolUse` hooks — [`permission-guard`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/permission-guard.sh) and [`wip-guard`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/wip-guard.sh) on `Bash`, and [`orchestrator-write-guard`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/orchestrator-write-guard.sh) on the file-writing tools — return a denial *before* the tool runs, and the command does not happen. The other six only report, and not for the same reason — a distinction this page owes you rather than one it can round off. Three of them [sit on events](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/hooks.json) that refuse nothing: `SessionStart` and `SubagentStart` have no tool call in front of them, so those three could not block if they wanted to. The other three could. `Stop` *can* block the turn from ending and `SubagentStop` *can* block a subagent from stopping, and every one of those scripts declines: every one of their exit paths is a success. [`zombie-loop-detect`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/zombie-loop-detect.sh) notices, one turn late and once per session for that head, that a branch is sitting on a gate verdict nobody acted on; [`dispatch-metrics-stop`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/dispatch-metrics-stop.sh) files the dispatch's numbers and gets out of the way; [`orchestrator-tool-census`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/orchestrator-tool-census.sh) names what the orchestrator did with its own hands, write and post calls apart from reads, and decides nothing about it. Three hooks choosing not to be mechanisms says more about this harness than one did. And the personas **advise** — their judgement is checked by nothing, and this repo's own guide says in as many words that a lens nobody dispatches [*fails silently*](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/commands/autonomy-on.md). That is the guarantee the loop gives — and it is worth exactly what the inventory in the drawing above is worth.
