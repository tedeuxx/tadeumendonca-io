...
| `content-reviewer` | raising a draft's bar before it reaches me — at most two rounds against `published-voice`, and it may block only where it can quote a clause of that skill | `content-writer`, and this is the roster's first true pair: everything it cannot quote comes back labelled advisory-and-droppable | `harness-engineering` · `command-hygiene` · `published-voice` |
