...
Two things in that last column are worth saying. **`harness-engineering` and `command-hygiene` are in all six** — the universal preload: understanding the loop itself, and the file-and-command discipline, belong to no specialty. And **only 7 of the [library's 13 skills](https://github.com/tedeuxx/tadeumendonca-skills/tree/main/skills) are preloaded by anyone**, which means more than half the library only reaches a session if the model finds it on its own, through its description.
