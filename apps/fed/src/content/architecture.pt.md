...
Por que o time de personas é de **cinco** e não de dezenove é uma decisão registrada, então vale a regra que rege esta página: apontar, não reescrever. O corte foi feito por quatro razões declaradas, e uma delas é que o contexto do orquestrador é um recurso finito que o desenho gasta de propósito — [as emendas do ADR-0002 no repositório do plugin](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/docs/adr/0002-agentic-dev-loop-architecture.md) trazem o critério, as razões e o que cada corte custou. O plural importa: *"cinco por causa da janela de contexto"* é uma simplificação que o próprio arquivo recusa.

Esse recurso agora foi medido uma vez, o que já é mais do que esta página conseguia dizer antes. Na sessão deste workspace de 7–8 de agosto de 2026, lida pelos transcripts da sessão: **125 acionamentos** — `quality-assurance` 45, `product-lead` 40, `tech-lead` 29, `harness-reviewer` 7, `developer` 2 — mantiveram cerca de **2,78 milhões de tokens dentro dos subagentes**, em 4.886 usos de ferramenta, e devolveram por volta de **159 mil tokens de texto de veredito**: 82 retornos, algo como 99 mil palavras. Razão agregada de **17×**; por acionamento, a mediana é **19×**. Os retornos foram uns **44%** de tudo que o orquestrador consumiu vindo de ferramenta.

> Uma tarefa custa ao orquestrador o **veredito**, não a **execução** — medido aqui em torno de um vinte avos. Isso é um limite, não uma fuga: 125 acionamentos devolveram ~99 mil palavras, e a sessão compactou duas vezes mesmo assim.

E é isso que transforma uma propriedade em botão. A economia é limitada pelo tamanho dos retornos, e esse é o único termo que o harness controla — escrevendo briefs de persona mais curtos.

Todo número acima é **uma leitura**: uma sessão, um repositório, um dia, um formato do time de personas. Não é propriedade do desenho, nada nesta página o recalcula, e os transcripts de onde ele saiu são locais e não publicados. Trate como a conta lá em cima é tratada — uma medição com data, não um fato permanente.
