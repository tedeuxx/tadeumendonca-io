...
flowchart LR
  ...
  subgraph COLD["NEGA"]
    direction TB
    HKD["3 hooks · dois eventos, dois formatos<br/>2 hooks · PreToolUse — recusam uma chamada<br/>matcher Bash · permission-guard<br/>matcher mcp · mcp-guard<br/>1 hook · UserPromptSubmit — recusa o prompt<br/>preflight"]
    PSD["— nenhuma persona"]
    SKD["— nenhuma skill"]
    CMD["— nenhum comando"]
    HKD ~~~ PSD ~~~ SKD ~~~ CMD
...
|---|---|---|---|
| `product-lead` | o leitor, valor, ordem, tamanho da fatia — e posicionamento, voz, e a verdade de qualquer coisa publicada | o `tech-lead`; e **barra um merge** diante de uma afirmação publicada que não é verdade — por convenção, não por hook | `agents-configuration` · `engineering-standards` · `definition-of-ready` · `shell` |
