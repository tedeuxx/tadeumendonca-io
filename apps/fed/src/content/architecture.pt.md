...
**Duas regras definem o que esse número é, e você merece tê-las antes dele.** Ele mede o que este site **acrescentou**, não aquilo de que ele **depende**: assinaturas que já existiam e cobrariam igual se ele fosse apagado amanhã ficam de fora. E ele mede aquilo em que o site **roda**, não aquilo com que eu o **construo** — são perguntas diferentes, e as duas subseções abaixo voltam em cada uma. Sem esse recorte, "USD 6,57" é um número solto, e um número solto não é conferível.
...
**A maior linha da conta é a que foi escolha — e um nome mais barato derruba o total.** Em vez de imprimir uma tabela de preços que envelhece, ou de citar uma comparação que eu não medi, fica o comando que responde isso para o nome que você quiser, pelo mesmo registrador que este usa: `aws route53domains list-prices --tld com`. Replique esta stack sob um nome mais barato e o número acima deixa de ser dominado por uma preferência minha.

`.io` é caro entre os domínios de topo, e eu escolhi por branding, não por custo — essa é a razão honesta, e é a única linha daqui que você pode recusar. Nada mais nesta conta se mexe com o nome: a hosted zone, o bucket e a distribuição não ligam para qual ele é.
...
A AWS é um dos fornecedores que poderiam faturar este site — e esse é o critério, declarado em vez de virar contagem, porque número envelhece na primeira dependência nova. **Entra aqui tudo que cobra para manter o site publicado no ar, ou que cobraria sob alguma condição.** O recorte é o segundo declarado lá em cima — o que o site **roda**, não aquilo com que eu o **construo** — e a seção final volta nele. Aplicado com honestidade, ele ainda revela mais do que uma lista de zeros — e o interessante é que os itens não se comportam do mesmo jeito.
