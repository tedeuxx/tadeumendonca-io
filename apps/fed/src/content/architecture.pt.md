...
flowchart TB
  ...
  subgraph AFK["AFK · do ready ao merge, nada no caminho é humano"]
    ...
    QA["TIER 3 · GATE<br/>— contexto fresco, sem viés de autoria<br/>quality-assurance · o único que pode fazer merge<br/>produto · conteúdo — a Definition of Done,<br/>e se isso quebra a produção<br/>loop — o marcador de veredito do agents-lead"]
...
flowchart LR
  ...
  accDescr: Uma grade: quatro raias, uma por tipo de componente que o plugin exporta, cruzadas com três colunas, uma por classe de força. As raias são os 9 hooks registrados no hooks.json, as 7 personas do diretório agents, as 14 skills do diretório skills e os 3 comandos do diretório commands. Das doze células, cinco têm conteúdo e sete estão vazias — e as vazias são a afirmação do desenho. Na coluna que nega há uma célula só: os 3 hooks do evento PreToolUse, que RECUSAM uma chamada de ferramenta antes dela rodar — permission-guard e wip-guard no matcher Bash, e orchestrator-write-guard nas ferramentas que escrevem arquivo, Edit, Write, MultiEdit e NotebookEdit; persona, skill e comando não têm célula nenhuma ali. Na coluna do meio também há uma só: as personas agents-lead, content-reviewer, content-writer, developer, product-lead, quality-assurance e tech-lead apenas ACONSELHAM, e isso é uma afirmação sobre o JULGAMENTO delas, não sobre a cadeira. quality-assurance é o caso mais agudo nas duas direções: a regra 7b do permission-guard recusa o comando de merge vindo de qualquer agent type que não seja esse, então QUEM faz o merge é forçado por mecanismo — e nada em lugar nenhum verifica se a revisão foi feita, ou feita bem. agents-lead é mais fraca ainda, e não pode ser lida como a mesma coisa: roda antes de qualquer coisa ser construída, não barra nada, e nada obriga que ela seja acionada — uma lente que não é acionada falha em silêncio. product-lead é o caso espelhado: BARRA um merge diante de uma afirmação publicada que não é verdade, mas por convenção e não por hook, então nada recusa o comando de merge em nome dela; e o par de conteúdo — content-writer, que redige texto publicado, e content-reviewer, que lê esse rascunho contra a mesma régua — é contido pelo mesmo mecanismo que contém product-lead: uma regra do permission-guard nega a cada um deles postar diretamente, já que os dois leem material privado pra fazer o que fazem. Na coluna dos que DOCUMENTAM ficam três células: as 14 skills, que é o que o modelo aciona sozinho, os 3 comandos, que é o que uma pessoa digita, e os outros 6 hooks — session-wip e session-plugin-version no SessionStart, dispatch-metrics-start e dispatch-metrics-stop no SubagentStart e no SubagentStop, e zombie-loop-detect e orchestrator-tool-census no Stop. Esses seis não estão nessa coluna pelo mesmo motivo, e dizer qual motivo vale pra qual é a afirmação mais afiada. Três deles não conseguem recusar nada: o SessionStart e o SubagentStart não entregam chamada nenhuma de ferramenta pra negar, então pro session-wip, pro session-plugin-version e pro dispatch-metrics-start a coluna diz um limite do EVENTO — um hook em qualquer um dos dois roda e pode agir, só não tem nada na frente pra barrar. Os outros três conseguiriam e não conseguem por escolha. O Stop e o SubagentStop CONSEGUEM recusar — o Claude Code deixa um hook no primeiro barrar o fim do turno e um hook no segundo barrar a parada do subagente — e nenhum dos scripts aqui aceita isso. O dispatch-metrics-stop escreve as métricas do acionamento na Issue dele e sai com sucesso em toda saída, inclusive nas que desistem cedo. O zombie-loop-detect roda no fim de todo turno meu e lê uma coisa só: o branch atual tem uma pull request aberta cujo último veredito do gate, conferido contra o head atual daquela pull request, pede mudanças ou espera o meu go? Se tem, o aviso abre o turno seguinte uma vez por sessão pra aquele head: não se repete enquanto nada se mexe, rearma quando entram commits novos, e uma sessão nova que pegue o mesmo branch fala de novo; toda saída dele é de sucesso e nada nunca é barrado. O orchestrator-tool-census é o mais novo deles e o instrumento mais simples: no fim de um turno ele me diz o que o orquestrador fez com as próprias mãos naquela sessão, como uma lista nomeada com as chamadas que escrevem e postam separadas das que só leem, e não barra nada, porque um hook no Stop dispara depois que o trabalho já aconteceu. Então esses três estão na coluna dos que DOCUMENTAM por ESCOLHA deles, e não por limite do evento, e o que o zombie-loop-detect compra é detecção um turno atrasada, não prevenção: um acionamento que foi narrado e nunca aconteceu deixa o loop parado sem nenhum artefato dizendo isso, e é ele o artefato que diz. Os seis só reportam — três porque o evento não permite outra coisa, três porque o script decidiu assim, e a coluna carrega os dois casos sem fingir que são um só. Lida de ponta a ponta, a grade é quase toda vazia, e é isso que ela afirma: de tudo que o plugin exporta, exatamente um tipo — um hook no PreToolUse — consegue recusar; todo o resto aconselha ou documenta.
  subgraph LANE["tipo · o que o -skills exporta"]
    direction TB
    LH["hooks · 9<br/>hooks.json"]
  ...
  subgraph COLD["NEGA"]
    direction TB
    HKD["3 hooks · PreToolUse<br/>matcher Bash<br/>permission-guard<br/>wip-guard<br/>matcher Edit Write MultiEdit NotebookEdit<br/>orchestrator-write-guard"]
    PSD["— nenhuma persona"]
    SKD["— nenhuma skill"]
    CMD["— nenhum comando"]
    HKD ~~~ PSD ~~~ SKD ~~~ CMD
  ...
  subgraph COLO["DOCUMENTA"]
    direction TB
    HKR["6 hooks · nenhum deles recusa<br/>2 hooks · SessionStart<br/>session-wip<br/>session-plugin-version<br/>2 hooks · SubagentStart e SubagentStop<br/>dispatch-metrics-start<br/>dispatch-metrics-stop<br/>2 hooks · Stop<br/>zombie-loop-detect<br/>orchestrator-tool-census<br/>Stop e SubagentStop poderiam barrar — nenhum barra"]
    PSO["— nenhuma persona"]
    SK["14 skills · skills/<br/>o que o modelo aciona"]
...
**Dos componentes do próprio plugin, exatamente um tipo consegue te barrar**, e essa é a versão honesta do convite a adotar: os três hooks de `PreToolUse` — o [`permission-guard`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/permission-guard.sh) e o [`wip-guard`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/wip-guard.sh) no `Bash`, e o [`orchestrator-write-guard`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/orchestrator-write-guard.sh) nas ferramentas que escrevem arquivo — devolvem uma negativa *antes* da ferramenta rodar, e o comando não acontece. Os outros seis só reportam, e não pelo mesmo motivo — e essa distinção esta página te deve, não dá pra arredondar. Três deles [estão em eventos](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/hooks.json) que não recusam nada: o `SessionStart` e o `SubagentStart` não têm chamada de ferramenta na frente, então esses três não conseguiriam barrar nem se quisessem. Os outros três conseguiriam. O `Stop` *consegue* barrar o fim do turno e o `SubagentStop` *consegue* barrar a parada de um subagente, e todos esses scripts escolhem não barrar: toda saída deles é de sucesso. O [`zombie-loop-detect`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/zombie-loop-detect.sh) percebe, um turno atrasado e uma vez por sessão pra aquele head, que tem branch parado em cima de um veredito do gate que ninguém tratou; o [`dispatch-metrics-stop`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/dispatch-metrics-stop.sh) registra os números do acionamento e sai da frente; o [`orchestrator-tool-census`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/scripts/orchestrator-tool-census.sh) nomeia o que o orquestrador fez com as próprias mãos, as chamadas que escrevem e postam separadas das que só leem, e não decide nada sobre isso. Três hooks escolhendo não ser mecanismo dizem mais sobre este harness do que um dizia. E as personas **aconselham** — o julgamento delas não é verificado por nada, e o guia deste repositório diz com todas as letras que uma lente que ninguém aciona [*falha em silêncio*](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/commands/autonomy-on.md). Essa é a garantia que o loop dá — e ela vale exatamente o que valer o inventário do desenho acima.
...
**[Sete personas](https://github.com/tedeuxx/tadeumendonca-skills/tree/main/agents), contra quem cada uma argumenta — e o que cada uma carrega ao ser acionada.** A última coluna é o preload de cada brief: as skills que entram na sessão da persona antes de ela ler a primeira linha da tarefa.

| quem | o que é dele | contra quem argumenta | que skills carrega ao ser acionada |
|---|---|---|---|
| `product-lead` | o leitor, valor, ordem, tamanho da fatia — e posicionamento, voz, e a verdade de qualquer coisa publicada | o `tech-lead`; e **barra um merge** diante de uma afirmação publicada que não é verdade — por convenção, não por hook | `harness-engineering` · `definition-of-ready` · `command-hygiene` |
| `tech-lead` | arquitetura, medição, sequenciamento — e é ele que escreve os registros de decisão de produto e de sistema; os do próprio loop não são dele, são do `agents-lead` | o `product-lead`, de propósito: produto-e-mercado e sistema são otimizações genuinamente diferentes | `harness-engineering` · `definition-of-ready` · `documentation-standard` · `devops` · `command-hygiene` |
...
| `content-writer` | redige artigos, texto do site e a linguagem dos posts de rede social na voz do dono — molda, corta, estrutura e traduz uma experiência que ele já tem, nunca origina uma | o `content-reviewer`, que lê o rascunho contra a mesma régua com que ele foi escrito; a verdade do que chega a uma superfície pública continua sendo o veto bloqueante do `product-lead`, no gate | `harness-engineering` · `command-hygiene` · `published-voice` |
| `content-reviewer` | levantar a régua de um rascunho antes de ele chegar a mim — no máximo duas rodadas contra o `published-voice`, e o que ele **barra** é um rascunho dentro dessas duas rodadas, nunca um merge, e só onde consegue citar uma cláusula dessa skill | o `content-writer`, e este é o primeiro par de verdade do elenco: tudo que ele não consegue citar volta rotulado como opinião descartável | `harness-engineering` · `command-hygiene` · `published-voice` |
| `agents-lead` | a maquinaria em si: hooks, permissões, instruções das personas, skills e comandos, o plugin — e é ele que escreve os registros de decisão sobre o loop, a outra metade da divisão acima | **eu** — e esse é o caso interessante: o contraponto dele não é outra persona, é a única cadeira deste loop que não tinha com quem discutir | `harness-engineering` · `documentation-standard` · `devops` · `command-hygiene` |

Duas coisas dessa última coluna valem ser ditas. **`harness-engineering` e `command-hygiene` estão nas sete** — é o preload universal: entender o próprio loop e a disciplina de arquivo e comando não é assunto de especialidade nenhuma. E **só 8 das [14 skills da biblioteca](https://github.com/tedeuxx/tadeumendonca-skills/tree/main/skills) são pré-carregadas por alguém**, ou seja, as outras seis só chegam numa sessão se o modelo as encontrar sozinho, pela descrição delas.
