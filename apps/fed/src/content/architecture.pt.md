...
### A parte que não vem na caixa

E tem um componente que não está em nada disso — nem no desenho, nem no manifesto, nem no plugin. O **orquestrador** é a sessão principal: o contexto que lê uma Issue, decide qual persona acionar, pesa o que volta e roda o merge. Instale o plugin e você recebe hooks, personas e comandos; quem junta tudo isso é a metade que você escreve, e quem está decidindo se adota isso merece saber qual das duas vem pronta. Ele é também a parte contra a qual as fronteiras de capacidade acima foram desenhadas — o `permission-guard` recusa o comando de merge vindo de qualquer agent type que não seja o `quality-assurance`, e a glosa na aresta das personas, *aconselha, se acionada*, nomeia o acionamento como o modo de falha sem nomear quem aciona. Quem aciona é o orquestrador, e uma lente que ele esquece é uma lente que ninguém rodou.

Por que o time de personas é de **cinco** e não de dezenove é uma decisão registrada, então vale a regra que rege esta página: apontar, não reescrever. O corte aconteceu numa emenda, e o critério por trás dele foi alargado numa emenda posterior — hoje o registro nomeia **quatro** razões pelas quais uma persona pode existir, e uma delas é que o contexto do orquestrador é um recurso finito que o desenho gasta de propósito. [As emendas do ADR-0002 no repositório do plugin](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/docs/adr/0002-agentic-dev-loop-architecture.md) trazem os dois movimentos e o que cada corte custou. O plural importa: *"cinco por causa da janela de contexto"* é uma simplificação que o próprio arquivo recusa.

Esse recurso foi lido uma vez, e a forma honesta dessa leitura é um **piso**, não um número. Medido na própria sessão deste repositório em 7–8 de agosto de 2026, pelos transcripts da sessão: ficou dentro dos subagentes **mais de uma ordem de grandeza** de contexto a mais do que voltou em forma de veredito. A economia é real e é limitada — a sessão **compactou duas vezes** assim mesmo, e os vereditos que voltaram foram uma **minoria grande** de tudo que o orquestrador consumiu vindo de ferramenta.

**Estimativas pontuais não são publicadas aqui de propósito, e a razão é a régua desta página, não modéstia.** Relido vinte e seis minutos depois, todo contador tinha se mexido, porque a população continua crescendo enquanto você mede; e há quatro denominadores diferentes em jogo — acionamentos, transcripts, agentes que retornaram, retornos — então qualquer razão única escolhe um deles em silêncio. Mais decisivo que os dois: **a entrada é um transcript de sessão privado, numa máquina só.** Não está em nenhum dos dois repositórios, gate nenhum alcança, e nada nesta página o recalcula. Isso faz dele uma leitura, e não um artefato que você possa conferir — e uma página que defende que afirmação se resolve em arquivo te deve essa frase quando uma não se resolve.

O que sobrevive à aritmética é o formato, e ele é uma afirmação de desenho, não uma medição:

> Uma tarefa custa ao orquestrador o **veredito**, não a **execução**. Isso é um limite, não uma fuga — os vereditos se acumulam do mesmo jeito, e esta sessão compactou duas vezes apesar disso.

E é isso que transforma uma propriedade em botão, e o botão também não precisa de número. A economia é limitada pelo tamanho dos vereditos, e esse é o único termo que o harness controla — escrevendo briefs de persona mais curtos.
...
**Nomear o orquestrador mexeu nessa conta, então ela vai de novo: três você confere, e agora dois você não.** O segundo é o orquestrador e tudo que foi dito sobre ele, e ele é inconferível em duas camadas. O ator em si não está em desenho nenhum nem em inventário nenhum — não é componente do plugin, então a verificação de deriva que mantém a figura do harness honesta é estruturalmente incapaz de enxergá-lo, e nada aqui fica vermelho quando aquela descrição deixa de ser verdade. E a leitura ao lado dele — declarada como piso justamente por isso — saiu de transcripts de sessão que estão na minha máquina e não são publicados, então o que você tem é depoimento com um método junto, não algo que dá pra rodar de novo. Escrito aqui em vez de ficar subentendido, porque isso muda uma conta que esta página publica sobre si mesma.
