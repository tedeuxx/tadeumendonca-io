...
E a versão estampada em tudo isso é conteúdo, e não encanamento de release: o que move cada dígito é uma decisão registrada — o tipo de coisa que esta página aponta em vez de reescrever.

*(→ [ADR-0044](https://github.com/tedeuxx/tadeumendonca-io/blob/main/docs/adr/0044-version-parts-deliberate-major-minor.md) os dígitos da versão carregam significado de conteúdo)*
...
**Nomear o orquestrador acrescentou uma afirmação que não tem desenho por trás, e este release acrescentou mais seis, então o que se conta aqui são afirmações e não figuras — onze agora: oito você confere, e três você não.** Cinco das seis novas se resolvem: a lista de funcionalidades em rotas que você abre e nas decisões ao lado delas, o rascunhador de publicação num script com testes próprios, o time de personas no registro do próprio plugin, o índice de documentação numa árvore viva, e o que os dígitos da versão significam no registro que os decide. A segunda que não dá pra conferir é o orquestrador e tudo que foi dito sobre ele, e ela deixa de se resolver em duas camadas. O ator em si não está em desenho nenhum daqui nem em inventário nenhum — não é componente do plugin, então a verificação de deriva que mantém a figura do harness honesta é estruturalmente incapaz de enxergá-lo, e nada aqui fica vermelho quando aquela descrição deixa de ser verdade. E a leitura ao lado dele — declarada como piso justamente por isso — saiu de transcrições de sessão que estão na minha máquina e não são publicadas, então o que você tem é depoimento com um método junto, não algo que dá pra rodar de novo.
