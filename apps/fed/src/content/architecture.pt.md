...
| `content-reviewer` | levantar a régua de um rascunho antes de ele chegar a mim — no máximo duas rodadas contra o `published-voice`, e só pode barrar onde consegue citar uma cláusula dessa skill | o `content-writer`, e este é o primeiro par de verdade do elenco: tudo que ele não consegue citar volta rotulado como opinião descartável | `harness-engineering` · `command-hygiene` · `published-voice` |
