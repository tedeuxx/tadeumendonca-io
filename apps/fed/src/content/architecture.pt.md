...
flowchart LR
  ...
  accDescr: A afirmação vem primeiro, porque é pra ela que a grade existe. De tudo que o plugin exporta, exatamente um tipo consegue recusar: só os hooks RECUSAM, e só em dois dos seis eventos em que eles se registram. As personas ACONSELHAM, e as skills e os comandos só DOCUMENTAM. Quatro tipos contra três classes de força dá doze células; cinco têm conteúdo e sete estão vazias, e as sete vazias são o argumento. Agora o inventário de onde a afirmação é lida. As raias são os 15 registros de hook no hooks.json, escritos por 14 scripts porque o preflight está registrado duas vezes; as 8 personas do diretório agents; as 15 skills do diretório skills; e os 7 comandos do diretório commands. Na coluna que nega há uma célula só, e ela tem 3 registros em dois eventos de formatos diferentes: o permission-guard no matcher Bash e o mcp-guard no matcher das ferramentas MCP, os dois no PreToolUse, que recusam uma chamada de ferramenta antes dela rodar; e o preflight no UserPromptSubmit, que recusa o próprio prompt, antes do turno começar. Persona, skill e comando não têm célula nenhuma ali. Na coluna do meio também há uma só: as 8 personas — agents-lead, content-reviewer, content-writer, developer, product-lead, quality-assurance, scrum-master e tech-lead — que ACONSELHAM, e isso é uma afirmação sobre o julgamento delas, não sobre a cadeira, porque uma lente que ninguém aciona falha em silêncio. A terceira coluna, a dos componentes que só DOCUMENTAM, tem três células: as 15 skills, que é o que o modelo aciona sozinho; os 7 comandos, que é o que uma pessoa digita; e os outros 12 registros de hook — 5 no SessionStart, 2 no SubagentStart e no SubagentStop, e 5 no Stop. Lidas as duas colunas juntas, o preflight aparece nas duas, porque está registrado duas vezes com uma classe diferente em cada evento, e é isso que significa o desenho ser derivado e não desenhado: ninguém escrevendo à mão desenharia um script em duas colunas da mesma grade.
  subgraph LANE["tipo · o que o -skills exporta"]
    direction TB
    LH["hooks · 15<br/>hooks.json<br/>14 scripts — um está registrado duas vezes"]
    LP["personas · 8<br/>agents/"]
    LS["skills · 15<br/>skills/"]
    LC["comandos · 7<br/>commands/"]
    LH ~~~ LP ~~~ LS ~~~ LC
  end
  subgraph COLD["NEGA"]
    direction TB
    HKD["3 hooks · dois eventos, dois formatos<br/>2 hooks · PreToolUse — recusam uma chamada<br/>matcher Bash · permission-guard<br/>matcher mcp__.* · mcp-guard<br/>1 hook · UserPromptSubmit — recusa o prompt<br/>preflight"]
    PSD["— nenhuma persona"]
    SKD["— nenhuma skill"]
    CMD["— nenhum comando"]
    HKD ~~~ PSD ~~~ SKD ~~~ CMD
  ...
  subgraph COLA["ACONSELHA"]
    direction TB
    HKA["— nenhum hook"]
    PS["8 personas · agents/<br/>agents-lead<br/>content-reviewer<br/>content-writer<br/>developer<br/>product-lead<br/>quality-assurance<br/>scrum-master<br/>tech-lead"]
    SKA["— nenhuma skill"]
    CMA["— nenhum comando"]
    HKA ~~~ PS ~~~ SKA ~~~ CMA
  end
  subgraph COLO["DOCUMENTA"]
    direction TB
    HKR["12 hooks · nenhum recusa onde está<br/>5 hooks · SessionStart<br/>preflight · session-wip · session-plugin-version · cadence-notice<br/>worktree-notice<br/>2 hooks · SubagentStart e SubagentStop<br/>dispatch-metrics-start · dispatch-metrics-stop<br/>5 hooks · Stop<br/>zombie-loop-detect · orchestrator-tool-census<br/>premature-pr-link-detect · owed-pr-link-detect<br/>closure-artifact-guard<br/>Stop e SubagentStop poderiam barrar — nenhum dos seis barra"]
    PSO["— nenhuma persona"]
    SK["15 skills · skills/<br/>o que o modelo aciona"]
    CM["7 comandos · commands/<br/>o que você digita<br/>autonomy · blueprint · funnel-review<br/>new-issue · sprint-planning<br/>sprint-review · sprint-retrospective"]
    HKR ~~~ PSO ~~~ SK ~~~ CM
  end
  LANE ~~~ COLD ~~~ COLA ~~~ COLO
...
**[Oito personas](https://github.com/tedeuxx/tadeumendonca-skills/tree/main/agents), contra quem cada uma argumenta — e o que cada uma carrega ao ser acionada.** A última coluna é o preload de cada brief: as skills que entram na sessão da persona antes de ela ler a primeira linha da tarefa.
...
**São 51 decisões, e agora este número também é mecânico.** O índice é **gerado** a partir de `docs/adr/`, commitado como artefato e conferido no CI: acrescentar ou substituir uma decisão sem regenerar o índice deixa o pipeline vermelho, então o artefato e o diretório não têm como se separar. Esta frase dizia que o número em si era a exceção — digitado à mão, sustentado só pelo link abaixo. Era — e deixou de ser verdade no mesmo dia. Foi escrita na manhã de 2026-08-25, com 49 registros no diretório; a **ADR-0050 entrou naquela noite**, e a página seguiu publicando `49` — a biblioteca já tinha 50, e 51 a partir da ADR-0051, em 2026-09-04 — até esta correção. Prever uma falha em prosa não é um controle, então o número agora é comparado com esse mesmo índice gerado por um teste, e os dois não têm mais como discordar sem o pipeline ficar vermelho antes. As linhas continuam não impressas aqui de propósito — esta página **aponta** o detalhe canônico em vez de repeti-lo, e uma cópia de 51 linhas seria a própria regra sendo quebrada na única seção que existe para defendê-la.
