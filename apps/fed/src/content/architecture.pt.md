...
- **[A pirâmide de perfis](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/docs/dev-loop-design.md)** — as seis personas não estão num mesmo plano: dois leads que discordam por construção em cima, junto do `harness-lead` — o par do dono na própria maquinaria, que não segura nenhum gate —, dois builders no meio, um gate embaixo. O que ela entrega é **discordância onde ela é útil e handoff onde não é** — e a regra que decide isso é explícita, custo de reconciliação se paga *dentro* de uma camada e não entre camadas, e é o que segurou o time em seis em vez de dezenove.
