...
- **Um plugin de dev-loop.** As personas, os hooks, as skills e os comandos que decidem *como* o trabalho é feito: versionado num repositório à parte, instalável em qualquer projeto, e o que ele entrega é uma verificação que não depende de quem escreveu o código.
...
O que fica na interseção é o trabalho de verdade: decidir o que o harness **barra**, o que ele **aconselha** e o que ele só **documenta** — e depois provar que o inventário disso continua verdadeiro. O `Agent` fica entre parênteses de propósito, e o parêntese não é um recuo: ele diz que a forma curta e a forma inteira nomeiam uma prática só, não duas. O lugar dele é onde o termo está sendo apresentado — o desenho acima, e este parágrafo. Mais abaixo, onde ele deixa de ser rótulo e vira a afirmação, o termo aparece por extenso: parêntese torna uma palavra opcional, e *agent* é justamente a palavra que não pode ser opcional na frase que diz que a afirmação é minha.
...
- **Dois dos seis hooks rodam no `PreToolUse`.** O runtime do agente chama eles *antes* da ferramenta rodar, eles devolvem uma negativa e o comando não acontece. **Eles são o piso.**
- **Os outros quatro rodam no `SessionStart`, no `SubagentStart` e no `SubagentStop`**, eventos que não entregam chamada nenhuma de ferramenta pra recusar, e é por isso que não estão desenhados como piso. **A classe diz** o que um hook nesses eventos *não consegue barrar*, e não que ele só observa — um hook em qualquer um deles roda e pode agir, só não tem chamada de ferramenta na frente pra recusar, e este desenho não tem forma pra isso. **As quatro só reportam:** `session-wip`, `session-plugin-version`, `dispatch-metrics-start` e `dispatch-metrics-stop`. Isso é um fato sobre cada script, não uma propriedade do evento, **e é por isso que** o desenho não pode ser lido como uma promessa sobre o que eles fazem.
...
- **As skills e os comandos não são nem uma coisa nem outra** — são a forma escrita de uma decisão já tomada, pra ninguém rediscutir ela às duas da manhã.
...
- **[A pirâmide de perfis](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/README.md)** — as seis personas não estão num mesmo plano: dois leads que discordam por construção em cima, junto do `harness-lead` — o par do dono na própria maquinaria, que não segura nenhum gate —, dois builders no meio, um gate embaixo. O que ela entrega é **discordância onde ela é útil e handoff onde não é** — e a regra que decide isso é explícita, custo de reconciliação se paga *dentro* de uma camada e não entre camadas, e é o que segurou o time em seis em vez de dezenove.
- **[Hooks](https://github.com/tedeuxx/tadeumendonca-skills/tree/main/hooks/scripts)** — a única parte do plugin que roda código e **recusa**. Dois respondem no `PreToolUse`, antes da ferramenta acontecer; os outros quatro só reportam — dois no `SessionStart`, um no `SubagentStart`, um no `SubagentStop`. O que eles entregam é o piso irreversível sem depender de alguém lembrar dele, e o registro de qual evento chama qual script está em [`hooks/hooks.json`](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/hooks/hooks.json).
- **[Skills e comandos](https://github.com/tedeuxx/tadeumendonca-skills/tree/main/skills)** — cada arquivo é uma decisão já tomada, escrita: qual serviço da AWS para qual cenário, que gate é bloqueante, como uma versão é cortada. **São dois diretórios com dois propósitos:** as 13 skills em `skills/` são o que o modelo aciona sozinho; os três comandos em [`commands/`](https://github.com/tedeuxx/tadeumendonca-skills/tree/main/commands) são o que uma pessoa digita — e são os únicos que declaram `argument-hint`, diferença que a suíte do próprio plugin cobra nas duas direções. **E essa diferença é semântica, não mecânica:** o runtime aciona e pré-carrega os dois diretórios igual, sem distinguir um do outro; eles existem separados para quem *lê* o repositório, e quem segura a separação é o teste, não o loader. As famílias existem na árvore e não no identificador: uma skill em `skills/infrastructure/cloud-infrastructure/` é acionada como `cloud-infrastructure`, sem o segmento da família. As que valem para qualquer repositório ficam em [`skills/principles`](https://github.com/tedeuxx/tadeumendonca-skills/tree/main/skills/principles). O que eles entregam é **ausência de re-decisão** — e como o plugin é instalado e não lido do disco, um dos hooks de sessão diz qual build está de fato rodando, porque uma skill corrigida e não recarregada é uma skill que não teve efeito.
...
| `harness-lead` | a maquinaria em si: hooks, permissões, instruções das personas, skills e comandos, o plugin — e, diferente do resto desta tabela, pode implementar as mudanças de harness que ela mesma revisa | **eu** — e esse é o caso interessante |

**O `harness-lead` é o que não cabe na regra como ela foi escrita primeiro**, e foi por isso que a regra foi alargada em vez de defendida. O contraponto dele não é outra persona; sou eu com o chapéu de engenheiro de harness, que é a única cadeira deste loop que não tinha com quem discutir. Efeito de segunda ordem de uma mudança de configuração é invisível de dentro da própria mudança — é essa a razão inteira de ele existir. Ele não barra nada, e nada obriga que seja acionado, então ele falha do mesmo jeito silencioso que toda lente daqui.

**O `writer` é o membro mais novo, e também não se encaixa na regra — dito sem forçar.** Ele existe por conta própria: um pedido do tipo `content` não tinha construtor mecânico antes dele, não porque discorda de alguém. O `product-lead` segura o que ele redige do mesmo jeito que segura qualquer outra afirmação publicada.

Os movimentos, e o que cada corte custou, estão registrados em vez de resumidos aqui: [as emendas do ADR-0002](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/docs/adr/0002-agentic-dev-loop-architecture.md) e [o desenho independente de harness](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/README.md), os dois no repositório do plugin. É a regra desta página aplicada de novo — apontar o detalhe canônico em vez de reescrevê-lo.
...
- **[o README do plugin](https://github.com/tedeuxx/tadeumendonca-skills/blob/main/README.md)** — o loop inteiro num arquivo só, e escrito sem depender de nenhum runtime de agente em particular: máquina de estados, roster, modelo de gates, formalismo de intake, Definition of Done, e o que viaja se o desenho mudar de harness. É o documento pra ler se você está adotando, e não inspecionando.
