...
Dizer "custo quase zero" é a coisa mais fácil desta página — e a mais fácil de ninguém conferir. Então segue o ritmo de gasto, lido do custo diário da conta no **fim de julho de 2026**, não estimado. Leia como medição com data, não como fato permanente — nenhuma fatura fechou nesse ritmo ainda:
...
Repare no desenho disso, porque ele é o honesto para um site estático: **o domínio custa mais do que servir o site.** Não existe linha de computação nenhuma — é isso que "sem backend" compra, e é por isso que o número quase não se mexe com dez ou dez mil visitantes.

### Pra que serve o guardrail, na prática

A mesma leitura mostrou cerca de **USD 12,80 por mês** que o site não estava usando: web ACLs de WAF e endereços IPv4 públicos ociosos, anexados a nada, esquecidos quando o backend foi aposentado. Vinte vezes o que o site custa. **Esses já saíram** — removidos em julho de 2026, e é a série diária de custo que confirma as cobranças pararem, não um console vazio dando a entender isso. Não é tudo: sobrou um resíduo menor da mesma época, ainda cobrando enquanto eu descubro o que ele guarda — esse é o estado honesto disto na hora em que escrevo.

Eu descobri lendo a fatura, o que é tarde. Então quem vigia agora é um orçamento no nível da conta, em `iac/budget.tf`, de propósito **não** escopado às tags deste projeto — se fosse, ele só enxergaria gasto que este repo criou, e este gasto era justamente do tipo que ele não criou. É essa a parte que se leva embora: infraestrutura que você para de usar não para de cobrar, e quem pega isso precisa estar olhando mais largo do que aquilo que você está construindo. *(→ [`iac/budget.tf`](https://github.com/tedeuxx/tadeumendonca-io/blob/main/iac/budget.tf))*
